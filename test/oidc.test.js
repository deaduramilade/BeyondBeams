...
test('verifies short-lived identity tokens and separates human, service, and agent identities', async () => {
  ...
  const verify = createOidcVerifier({ issuer: 'https://identity.example.invalid', audience: 'oblivion-api', jwksClient: client, now: () => now });
  for (const type of ['human', 'service', 'agent']) {
    const principal = await verify(token({ iss: 'https://identity.example.invalid', aud: 'oblivion-api', sub: `${type}-subject`, tenant_id: 'tenant-a', workload_id: `${type}-workload`, principal_type: type, scope: 'action:realtime.defense.breach.detect', iat: 1700000000, exp: 1700000060 }));
...
test('rejects wrong audience, expired/revoked tokens, unknown keys, and unavailable identity provider', async () => {
  ...
  const client = new JwksClient({ uri: 'https://identity.example.invalid/jwks', fetch: async () => ({ ok: true, json: async () => ({ keys: [jwk] }) }), now: () => now });
  const verify = createOidcVerifier({ issuer: claims.iss, audience: 'oblivion-api', jwksClient: client, now: () => now });
  await assert.rejects(() => verify(token(claims)), { code: 'INVALID_TOKEN_CLAIMS' });
  const revoked = createOidcVerifier({ issuer: claims.iss, audience: 'oblivion-api', jwksClient: client, now: () => now, isRevoked: async () => true });
  await assert.rejects(() => revoked(token({ ...claims, aud: 'oblivion-api' })), { code: 'REVOKED_TOKEN' });
  await assert.rejects(() => verify(token({ ...claims, aud: 'oblivion-api' }, { alg: 'ES256', kid: 'unknown', typ: 'at+jwt' })), { code: 'UNKNOWN_IDENTITY_KEY' });
